...
procedure bajaFisica(var arch_emp: archivo_empleados);
var
    pos: integer;
    emp: empleado;
    aux: empleado;
begin
    writeln('Ingrese una posicion del archivo a borrar');
    readln(pos);
    Reset(arch_emp);
    leer(arch_emp, emp);
    while (emp.numero <> valorAlto) do
        begin
            if (filepos(arch_emp) = pos) then
                begin
                    seek(arch_emp, FileSize(arch_emp));
                    read(arch_emp, aux);
                    seek(arch_emp, pos);
        ...
        begin
            ...
            if (opcion = 'A') then
                ...
                begin
                    ...
                    writeln('E: Eliminar un empleado del archivo');
                    write('Opcion '); readln(opcion);
                    writeln();
                    case opcion of
                        'A' : ej4opA(arch_emp);
